...
create_url_unicef <- function(indicator) {
  ...
  cd2030 <- c(
    "SH.MLR.NETS.ZS", "SH.STA.ANV4.ZS", "SH.STA.ORTH",
  ...
  # PT - Child Protection
  pt_cp <- c(
    "SP.REG.BRTH.ZS", "SP.REG.BRTH.UR.ZS", "SP.REG.BRTH.RU.ZS",
    "SP.REG.BRTH.MA.ZS", "SP.REG.BRTH.FE.ZS", "SP.M18.2024.FE.ZS",
    "SP.M15.2024.FE.ZS"
  )
  # HIV_AIDS - HIV/AIDS
  hiv_aids <- c(
    "SH.HIV.KNOW.FE.ZS", "SH.HIV.KNOW.MA.ZS",
...
apply_filter_unicef <- function(df) {
  ...
  dhs_mics <-
    c(
      "SH.STA.ARIC.ZS", "SH.STA.ORCF.ZS", "SH.STA.ORTH",
  ...
  if (any(df$indicator %in% dhs_mics)) {
    df <- filter_unicef(df,
      priority =
        c(
          "DHS|[Dd]emographic and [Hh]ealth [Ss]urvey", #|Demographic Survey'
